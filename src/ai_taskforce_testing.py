...
# make the object that is used to make the API calls
client = openai.OpenAI(
    base_url="https://api.genai.mil/v1",
    api_key=os.getenv("GENAI_KEY")
)

# make the message to be sent through the API
message = f"""
...
# Send the message and receive the response from the API
...
# iterate over the responses from the API to find a valid response
valid = -1
for choice in range (n):
    try:
        # run the code generated by gen AI
